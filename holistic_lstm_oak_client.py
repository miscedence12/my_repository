...
lstm_classifier = ActionClassificationLSTM.load_from_checkpoint("lightning_logs/version_10\checkpoints\epoch=70-step=24210.ckpt")
        ...
        if data_json["start"]==1:
            ...
            with dai.Device(pipeline) as device:
                ...
                while not isExit:
                    ...
                    if data_json['start']==0:
                        logging.info("关闭条件满足")
                        device.close()
                        cv.destroyAllWindows()
                        break
                    if data_json["start"] not in [0,1]:
