...
# f=open("./output/停止.txt", "a", encoding="utf-8")
f=open("./output/X_train.txt", "a", encoding="utf-8")
...
def main():
    ...
    # カメラ準備 ###############################################################
    cap = cv.VideoCapture("output/train/b2.avi")
    ...
    while True:
        ...
        if not ret:
            break
        image = cv.flip(image, 1)  # ミラー表示  水平翻转
        ...
        if pose_landmarks is not None:
            ...
            for index, landmark in enumerate(pose_landmarks.landmark):
                if index==12 or index==14 or index==16:
                    ...
                    a.append(str(landmark_x))
                    a.append(",")
                    a.append(str(landmark_y) + ",")
        ...
        if right_hand_landmarks is not None:
            ...
            frames+=1
            if fram_int%2==0:
                b=int(fram_int-1) if int(fram_int-1)%2==0 else int(fram_int-2)
            else:
                b=fram_int-3
            # print(b)
            # print(frames)
